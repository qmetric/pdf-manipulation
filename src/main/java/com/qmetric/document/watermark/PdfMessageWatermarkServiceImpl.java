...
public class PdfMessageWatermarkServiceImpl implements PdfMessageWatermarkService
{
    private final MessageWatermarkStrategyFactory messageWatermarkStrategyFactory;

    private final PdfWatermarkService pdfWatermarkService;

    public PdfMessageWatermarkServiceImpl(final MessageWatermarkStrategyFactory messageWatermarkStrategyFactory, final PdfWatermarkService pdfWatermarkService)
    {
        this.messageWatermarkStrategyFactory = messageWatermarkStrategyFactory;
        this.pdfWatermarkService = pdfWatermarkService;
    }

    @Override public void watermark(final String documentSourcePath, final String documentOutputPath, final String watermarkText) throws Exception
    {
        final MessageWatermarkStrategy messageWatermarkStrategy = messageWatermarkStrategyFactory.strategyFor(watermarkText);
        
        pdfWatermarkService.watermark(documentSourcePath, documentOutputPath, messageWatermarkStrategy);
    }
}
