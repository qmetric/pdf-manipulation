...
public class ITextWatermark implements Watermark {
    @Override
    public byte[] watermark(byte[] originalPdf, byte[] watermarkImage, byte[] password) {

        try(ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
            Image watermark = Image.getInstance(watermarkImage);
            watermark.setAbsolutePosition(0, 0);

            PdfReader original = new PdfReader(originalPdf, password);

            PdfStamper stamper = new PdfStamper(original, outputStream);

            int i = 0;
            PdfContentByte pdfContentByte;
            while (i < original.getNumberOfPages())
            {
                i++;
                pdfContentByte = stamper.getUnderContent(i);
                pdfContentByte.addImage(watermark);
            }
            ...
            final byte[] watermarked = outputStream.toByteArray();

            return new ITextPasswordProtection().addProtection(watermarked, password);
        } catch (Exception e) {
