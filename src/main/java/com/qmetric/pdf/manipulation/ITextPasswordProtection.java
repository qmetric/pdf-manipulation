...
public class ITextPasswordProtection implements PasswordProtection {

    @Override
    public byte[] removeProtection(byte[] originalPdf, byte[] password) {
        try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
            final PdfReader reader = new PdfReader(originalPdf, password);
    ...
    @Override
    public byte[] addProtection(byte[] originalPdf, byte[] password) {
        try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
            PdfReader reader = new PdfReader(originalPdf);

            final PdfStamper stamper = new PdfStamper(reader, outputStream);

            stamper.setEncryption(password, password, PdfWriter.ALLOW_PRINTING, PdfWriter.ENCRYPTION_AES_128);
