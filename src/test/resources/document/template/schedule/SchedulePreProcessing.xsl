...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="DocumentDetails">
        <document-details>
            <product>
                <xsl:value-of select="scheduleDetails/policyDetails/product"/>
            </product>
            <external-images>
                <xsl:value-of select="scheduleDetails/resources/baseAssetPath"/>
            </external-images>
            <insurer-logo-name>
                <xsl:value-of select="scheduleDetails/policyDetails/insurer"/>
            </insurer-logo-name>
    ...
    <xsl:template name="DocumentTitle">
        <title>
            <xsl:apply-templates select="scheduleDetails/policyDetails/product"/>
    ...
    <xsl:template name="ReferenceSection">
        <references>
            <account-number>
                ...
                <number>
                    <xsl:apply-templates select="scheduleDetails/policyHolderDetails/customerId"/>
                </number>
            </account-number>
            <policy-number>
                <label>
                ...
                <number>
                    <xsl:apply-templates select="scheduleDetails/policyDetails/policyNumber"/>
                </number>
            </policy-number>
        </references>
    </xsl:template>
        ...
        <policy-information-table>
            ...
            <table-body>
                <row>
                    ...
                    <entry>
                        <xsl:apply-templates select="scheduleDetails/policyDetails/insurerName"/>
                    </entry>
                </row>
                <row>
                    ...
                    <entry>
                        <xsl:apply-templates select="scheduleDetails/policyDetails/product"/>
                        <xsl:text> Insurance - </xsl:text>
                        <xsl:apply-templates select="scheduleDetails/policyDetails/productClass"
                                />
                    </entry>
                </row>
                <row>
                    ...
                    <entry>
                        ...
                        <xsl:apply-templates select="scheduleDetails/policyDetails/startDate"/>
                        <xsl:text> at </xsl:text>
                        <xsl:apply-templates select="scheduleDetails/policyDetails/startTime"/>
                        <xsl:text> to </xsl:text>
                        <xsl:apply-templates select="scheduleDetails/policyDetails/expiryDate"/>
                        <xsl:text> at </xsl:text>
                        <xsl:apply-templates select="scheduleDetails/policyDetails/expiryTime"/>
                    </entry>
                </row>
                <row>
                ...
                <row>
                    ...
                    <entry>
                        <xsl:apply-templates select="scheduleDetails/policyDetails/issueDate"/>
                    </entry>
                </row>
                <row>
                    ...
                    <entry>
                        ...
                        <xsl:apply-templates select="scheduleDetails/policyDetails/totalCost"/>
                        <xsl:text> which includes Insurance Premium Tax of </xsl:text>
                        <xsl:apply-templates select="scheduleDetails/policyDetails/iptAmount"/>
                        <xsl:text> at </xsl:text>
                        <xsl:apply-templates select="scheduleDetails/policyDetails/iptPercentage"/>
                        <xsl:text>%</xsl:text>
                    </entry>
                </row>
            </table-body>
    ...
    <xsl:template name="PolicyholderInfo">
        <policyholder-info-table>
            ...
            <table-body>
                <row>
                    <entry>
                        <xsl:apply-templates select="scheduleDetails/policyHolderDetails/title"/>
                        <xsl:apply-templates select="scheduleDetails/policyHolderDetails/firstName"/>
                        <xsl:apply-templates select="scheduleDetails/policyHolderDetails/lastName"/>
                    </entry>
                    <entry>
                        <xsl:apply-templates select="scheduleDetails/policyHolderOccupation"/>
                    </entry>
                </row>
                <xsl:apply-templates select="scheduleDetails/jointPolicyHolders/jointPolicyHolder"/>
            </table-body>
        </policyholder-info-table>
    </xsl:template>
        ...
        <personal-possessions>
            <xsl:if test="additionalItems[count(*)gt 0] or valuableItems[count(*)gt 0]">
                <specified-items-table>
                    <title><xsl:apply-templates select="nameForValuables"/> you have specified</title>
                    <table-header>
                        <row>
            ...
            <table-body>
                ...
                <row>
                    ...
                    <entry>
                        ...
                        <para>It is a condition of purchasing and renewing your policy that you agree to the information on your application form and any incidents
                ...
                <row>
                    ...
                    <entry>
                        <para>Remember you are responsible for the accuracy of the answers on all of your policy documentation, not the agent or
                            the company. Please ensure that all questions are fully and correctly answered.</para>
                        <para>I/We agree that the answers given on all of the policy documentation have been carefully checked and that if any
                            answer has been given by any other person such person shall for the purpose be regarded as my/our agent and not the agent of the company.</para>
                        <para>I/We declare that to the best of my/our knowledge and belief the answers on all of the policy documentation are true and correct and
                            any material fact as explained above has been disclosed.</para>
                        <para>I/We consent to the seeking of information from other insurers to check the answers I/we have provided, and I/we authorise the giving of such
                            information for such purposes.</para>
                        <para>I/We declare that the maximum claim limit(s) represent the full rebuilding cost of the Buildings and the full replacement value of the
                            Contents to be insured.</para>
                        <para>I/We agree that this insurance declaration shall be the basis of the contract between me/us and the company and I/we agree to be bound by
                            the terms, conditions and exceptions of the company's policy.</para>
                    </entry>
                </row>
                <row>
    ...
    <xsl:template match="policyHolderDetails/title | jointPolicyHolder/title">
        <xsl:apply-templates/>
        <xsl:text> </xsl:text>
    </xsl:template>

    <xsl:template match="policyHolderDetails/firstName | jointPolicyHolder/firstName">
        <xsl:apply-templates/>
        <xsl:text> </xsl:text>
    </xsl:template>

    <xsl:template match="policyHolderDetails/lastName | jointPolicyHolder/lastName">
        <xsl:apply-templates/>
    </xsl:template>

    <xsl:template match="policyHolderOccupation | jointPolicyHolder/occupation">
        <xsl:apply-templates/>
    </xsl:template>
    ...
    <xsl:template match="possessionsStandardCover">
        <standard-cover-table>
            <title><xsl:apply-templates select="../nameForValuables"/> - cover in the home</title>
            <table-header>
                <row>
                    <entry>Item type</entry>
    ...
    <xsl:template match="nameForValuables">
        <xsl:apply-templates/>
    </xsl:template>
