...
public class ImageWatermarkStrategyTest
{
    private PdfWatermarkFactory watermarkFactory = mock(PdfWatermarkFactory.class);

    private final ImageWatermarkStrategy imageWatermarkStrategy = new ImageWatermarkStrategy(watermarkFactory);

    private final PdfReader reader = mock(PdfReader.class);

    private final PdfStamper outputPdf = mock(PdfStamper.class);

    @Test
    public void shouldCreateImageFromFactory() throws Exception
    {
        when(reader.getNumberOfPages()).thenReturn(0);
        
        imageWatermarkStrategy.apply(reader, outputPdf);
        
        verify(watermarkFactory).createImage(reader);
    }
    
    @Test
    public void shouldAddImageToEachPage() throws Exception
    {
        final Image image = mock(Image.class);
        
        when(reader.getNumberOfPages()).thenReturn(2);
        when(watermarkFactory.createImage(reader)).thenReturn(image);

        final PdfContentByte contentPageOne = mock(PdfContentByte.class);
        final PdfContentByte contentPageTwo = mock(PdfContentByte.class);
        ...
        verify(contentPageOne).addImage(image);
        verify(contentPageTwo).addImage(image);
    }
}
